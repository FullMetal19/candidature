...
$req = mysqli_query($con,"SELECT * FROM ec_connexion_per WHERE mail='$per_email'");
...
if(!empty($per_nom) && !empty($per_prenom) && !empty($per_matricule) && !empty($per_email) && !empty($per_ufr) && !empty($per_mot_de_passe)){
    ...
    $req = mysqli_query($con,"INSERT INTO ec_connexion_per VALUES('$per_matricule','$per_nom','$per_prenom','$per_ufr','$per_email','$per_mot_de_passe','0')");
